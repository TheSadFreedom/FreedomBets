...
Таблица ставок **текущего профиля** с фильтрами и колонками: дата, ивент, команды, тип, сумма, кэф, результат, статус.
...
- ивент (логотип организатора, название);
...
| Tier | Где в UI |
|------|----------|
| `Small` | Вкладка «Ивенты» |
| `Big` | Вкладка «Ивенты» |
| `Major` | Вкладка «Major» |

### `EventStats`

Группирует ставки профиля по паре `(eventOrganization, eventName)`, считает метрики, показывает карточки ивентов.

Admin может редактировать ивент (даты, tier) → `updateEvent` обновляет связанные ставки и матчи с тем же ивентом.
...
1. Добавить Major (выбор из существующих ивентов в ставках).
...
| Элемент | Файл |
|---------|------|
| «Новый матч», «Новый ивент» | `HomeQuickActions` |
| Редактирование / удаление матча | `MatchCard` |
| Редактирование ивента | `EventStats`, `MajorEventStats` |
...
3. Те же **ивент** (организатор + название, без лишних пробелов).
